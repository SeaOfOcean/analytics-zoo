...
class BboxPred(inputSize: Int,
  ...
  bRegularizer: Regularizer[Float] = null, nClass: Int)(implicit ev: TensorNumeric[Float]) extends Linear(
  inputSize, outputSize, withBias, wRegularizer, bRegularizer) {

  private var normalized = false
  private val means = FasterRcnnParam.BBOX_NORMALIZE_MEANS.reshape(Array(1, 4))
    .expand(Array(nClass, 4)).reshape(Array(nClass * 4))
  private val stds = FasterRcnnParam.BBOX_NORMALIZE_STDS.reshape(Array(1, 4))
    .expand(Array(nClass, 4)).reshape(Array(nClass * 4))
  private var originalWeight = Tensor[Float](1)
  private var originalBias = Tensor[Float](1)
