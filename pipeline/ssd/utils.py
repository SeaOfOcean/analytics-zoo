...
def decode_detections(output, sizes):
    if sizes:
        return output.zip(sizes).map(lambda (output_i, img_size): decode_detection(output_i, img_size))
    else:
        return output.map(lambda output_i: decode_detection(output_i, None))

# scale normalized results from (0, 1) to original image size
def decode_detection(output, img_size):
    total = int(output[0])
